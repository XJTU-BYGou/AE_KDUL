...
args = sn.io.parse_arguments('.\PSS_newtestPSD.yaml')
...
dataFile = h5py.File('.\testingData_scaNet.mat','r')
data = np.transpose(dataFile['wfData'])
times = np.transpose(dataFile['wfTime'])
...
with tf.Session(graph=g, config=config) as sess:
    ...
    with g.as_default():
        ...
        loss, cat = models.gmm(mu, cov, tau, sx_proj, **args['gmm'])
        ...
        for epoch in range(epochs):
            ...
            for b in range(n_batches+1):
                ...
                if x_batch.shape[0]< batch_size:
                    real_size = x_batch.shape[0]
                    repeatNum = np.ones(real_size,dtype='int64')
                    repeatNum[-1] = batch_size - real_size + 1
                    x_batch = x_batch.repeat(repeatNum,axis=0)
            ...
            scat_all = scat_all.reshape(scat_all.shape[0], -1)
            scat_all = scat_all[:,0:data.shape[0]].T
