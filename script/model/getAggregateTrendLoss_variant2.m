function loss = getAggregateTrendLoss_variant2(Timefun,input,varargin)
delta = 1e-8;
reptNum = 1;
traveNum = 20;
w2 = 0.5;
...
[~,predL] = max(input,[],1);
onehotY = predL == [1;2];
if isdlarray(onehotY)
    onehotY = extractdata(onehotY);
end
if isdlarray(input)
    inputValue = extractdata(input);
else
    inputValue = input;
end
C_p1 = -timeTransMat.Mat  * (input(1,:).*onehotY(1,:)./(inputValue(1,:)+1e-8))';
C_p2 = -timeTransMat.Mat  * (input(2,:).*onehotY(2,:)./(inputValue(2,:)+1e-8))';
...
for i = 1:traveNum
    ...
    R1 = interruptMat * C_p1 ;
    R2 = interruptMat * C_p2 ;
    % Time intervals
